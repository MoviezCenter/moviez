...
func (mq *MovieQuery) loadReviews(ctx context.Context, query *ReviewQuery, nodes []*Movie, init func(*Movie), assign func(*Movie, *Review)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(review.FieldMovieID)
	}
	query.Where(predicate.Review(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(movie.ReviewsColumn), fks...))
	}))
...
func (mq *MovieQuery) loadGenres(ctx context.Context, query *GenreQuery, nodes []*Movie, init func(*Movie), assign func(*Movie, *Genre)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(genre.FieldID), joinT.C(movie.GenresPrimaryKey[0]))
