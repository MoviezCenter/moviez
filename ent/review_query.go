...
func (rq *ReviewQuery) QueryCreator() *UserQuery {
...
func (rq *ReviewQuery) QueryMovie() *MovieQuery {
	query := (&MovieClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ReviewQuery) WithCreator(opts ...func(*UserQuery)) *ReviewQuery {
	query := (&UserClient{config: rq.config}).Query()
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	...
	if query := rq.withCreator; query != nil {
		if err := rq.loadCreator(ctx, query, nodes, nil,
			func(n *Review, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
...
func (rq *ReviewQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Review, init func(*Review), assign func(*Review, *User)) error {
...
func (rq *ReviewQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withCreator != nil {
			_spec.Node.AddColumnOnce(review.FieldCreatorID)
		}
		if rq.withMovie != nil {
			_spec.Node.AddColumnOnce(review.FieldMovieID)
		}
	}
