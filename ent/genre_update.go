// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"errors"
	"fmt"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/MoviezCenter/moviez/ent/genre"
	"github.com/MoviezCenter/moviez/ent/movie"
	"github.com/MoviezCenter/moviez/ent/predicate"
)

// GenreUpdate is the builder for updating Genre entities.
type GenreUpdate struct {
	config
	hooks    []Hook
	mutation *GenreMutation
}

// Where appends a list predicates to the GenreUpdate builder.
func (gu *GenreUpdate) Where(ps ...predicate.Genre) *GenreUpdate {
	gu.mutation.Where(ps...)
	return gu
}

// SetName sets the "name" field.
func (gu *GenreUpdate) SetName(s string) *GenreUpdate {
	gu.mutation.SetName(s)
	return gu
}

// SetNillableName sets the "name" field if the given value is not nil.
func (gu *GenreUpdate) SetNillableName(s *string) *GenreUpdate {
	if s != nil {
		gu.SetName(*s)
	}
	return gu
}

// SetTypeID sets the "type_id" field.
func (gu *GenreUpdate) SetTypeID(u uint32) *GenreUpdate {
	gu.mutation.ResetTypeID()
	gu.mutation.SetTypeID(u)
	return gu
}

// SetNillableTypeID sets the "type_id" field if the given value is not nil.
func (gu *GenreUpdate) SetNillableTypeID(u *uint32) *GenreUpdate {
	if u != nil {
		gu.SetTypeID(*u)
	}
	return gu
}

// AddTypeID adds u to the "type_id" field.
func (gu *GenreUpdate) AddTypeID(u int32) *GenreUpdate {
	gu.mutation.AddTypeID(u)
	return gu
}

// AddMovieIDs adds the "movies" edge to the Movie entity by IDs.
func (gu *GenreUpdate) AddMovieIDs(ids ...int) *GenreUpdate {
	gu.mutation.AddMovieIDs(ids...)
	return gu
}

// AddMovies adds the "movies" edges to the Movie entity.
func (gu *GenreUpdate) AddMovies(m ...*Movie) *GenreUpdate {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return gu.AddMovieIDs(ids...)
}

// Mutation returns the GenreMutation object of the builder.
func (gu *GenreUpdate) Mutation() *GenreMutation {
	return gu.mutation
}

// ClearMovies clears all "movies" edges to the Movie entity.
func (gu *GenreUpdate) ClearMovies() *GenreUpdate {
	gu.mutation.ClearMovies()
	return gu
}

// RemoveMovieIDs removes the "movies" edge to Movie entities by IDs.
func (gu *GenreUpdate) RemoveMovieIDs(ids ...int) *GenreUpdate {
	gu.mutation.RemoveMovieIDs(ids...)
	return gu
}

// RemoveMovies removes "movies" edges to Movie entities.
func (gu *GenreUpdate) RemoveMovies(m ...*Movie) *GenreUpdate {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return gu.RemoveMovieIDs(ids...)
}

// Save executes the query and returns the number of nodes affected by the update operation.
func (gu *GenreUpdate) Save(ctx context.Context) (int, error) {
	return withHooks(ctx, gu.sqlSave, gu.mutation, gu.hooks)
}

// SaveX is like Save, but panics if an error occurs.
func (gu *GenreUpdate) SaveX(ctx context.Context) int {
	affected, err := gu.Save(ctx)
	if err != nil {
		panic(err)
	}
	return affected
}

// Exec executes the query.
func (gu *GenreUpdate) Exec(ctx context.Context) error {
	_, err := gu.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (gu *GenreUpdate) ExecX(ctx context.Context) {
	if err := gu.Exec(ctx); err != nil {
		panic(err)
	}
}

func (gu *GenreUpdate) sqlSave(ctx context.Context) (n int, err error) {
	_spec := sqlgraph.NewUpdateSpec(genre.Table, genre.Columns, sqlgraph.NewFieldSpec(genre.FieldID, field.TypeInt))
	if ps := gu.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if value, ok := gu.mutation.Name(); ok {
		_spec.SetField(genre.FieldName, field.TypeString, value)
	}
	if value, ok := gu.mutation.TypeID(); ok {
		_spec.SetField(genre.FieldTypeID, field.TypeUint32, value)
	}
	if value, ok := gu.mutation.AddedTypeID(); ok {
		_spec.AddField(genre.FieldTypeID, field.TypeUint32, value)
	}
	if gu.mutation.MoviesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   genre.MoviesTable,
			Columns: genre.MoviesPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(movie.FieldID, field.TypeInt),
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := gu.mutation.RemovedMoviesIDs(); len(nodes) > 0 && !gu.mutation.MoviesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   genre.MoviesTable,
			Columns: genre.MoviesPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(movie.FieldID, field.TypeInt),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := gu.mutation.MoviesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   genre.MoviesTable,
			Columns: genre.MoviesPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(movie.FieldID, field.TypeInt),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	if n, err = sqlgraph.UpdateNodes(ctx, gu.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{genre.Label}
		} else if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{msg: err.Error(), wrap: err}
		}
		return 0, err
	}
	gu.mutation.done = true
	return n, nil
}

// GenreUpdateOne is the builder for updating a single Genre entity.
type GenreUpdateOne struct {
	config
	fields   []string
	hooks    []Hook
	mutation *GenreMutation
}

// SetName sets the "name" field.
func (guo *GenreUpdateOne) SetName(s string) *GenreUpdateOne {
	guo.mutation.SetName(s)
	return guo
}

// SetNillableName sets the "name" field if the given value is not nil.
func (guo *GenreUpdateOne) SetNillableName(s *string) *GenreUpdateOne {
	if s != nil {
		guo.SetName(*s)
	}
	return guo
}

// SetTypeID sets the "type_id" field.
func (guo *GenreUpdateOne) SetTypeID(u uint32) *GenreUpdateOne {
	guo.mutation.ResetTypeID()
	guo.mutation.SetTypeID(u)
	return guo
}

// SetNillableTypeID sets the "type_id" field if the given value is not nil.
func (guo *GenreUpdateOne) SetNillableTypeID(u *uint32) *GenreUpdateOne {
	if u != nil {
		guo.SetTypeID(*u)
	}
	return guo
}

// AddTypeID adds u to the "type_id" field.
func (guo *GenreUpdateOne) AddTypeID(u int32) *GenreUpdateOne {
	guo.mutation.AddTypeID(u)
	return guo
}

// AddMovieIDs adds the "movies" edge to the Movie entity by IDs.
func (guo *GenreUpdateOne) AddMovieIDs(ids ...int) *GenreUpdateOne {
	guo.mutation.AddMovieIDs(ids...)
	return guo
}

// AddMovies adds the "movies" edges to the Movie entity.
func (guo *GenreUpdateOne) AddMovies(m ...*Movie) *GenreUpdateOne {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return guo.AddMovieIDs(ids...)
}

// Mutation returns the GenreMutation object of the builder.
func (guo *GenreUpdateOne) Mutation() *GenreMutation {
	return guo.mutation
}

// ClearMovies clears all "movies" edges to the Movie entity.
func (guo *GenreUpdateOne) ClearMovies() *GenreUpdateOne {
	guo.mutation.ClearMovies()
	return guo
}

// RemoveMovieIDs removes the "movies" edge to Movie entities by IDs.
func (guo *GenreUpdateOne) RemoveMovieIDs(ids ...int) *GenreUpdateOne {
	guo.mutation.RemoveMovieIDs(ids...)
	return guo
}

// RemoveMovies removes "movies" edges to Movie entities.
func (guo *GenreUpdateOne) RemoveMovies(m ...*Movie) *GenreUpdateOne {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return guo.RemoveMovieIDs(ids...)
}

// Where appends a list predicates to the GenreUpdate builder.
func (guo *GenreUpdateOne) Where(ps ...predicate.Genre) *GenreUpdateOne {
	guo.mutation.Where(ps...)
	return guo
}

// Select allows selecting one or more fields (columns) of the returned entity.
// The default is selecting all fields defined in the entity schema.
func (guo *GenreUpdateOne) Select(field string, fields ...string) *GenreUpdateOne {
	guo.fields = append([]string{field}, fields...)
	return guo
}

// Save executes the query and returns the updated Genre entity.
func (guo *GenreUpdateOne) Save(ctx context.Context) (*Genre, error) {
	return withHooks(ctx, guo.sqlSave, guo.mutation, guo.hooks)
}

// SaveX is like Save, but panics if an error occurs.
func (guo *GenreUpdateOne) SaveX(ctx context.Context) *Genre {
	node, err := guo.Save(ctx)
	if err != nil {
		panic(err)
	}
	return node
}

// Exec executes the query on the entity.
func (guo *GenreUpdateOne) Exec(ctx context.Context) error {
	_, err := guo.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (guo *GenreUpdateOne) ExecX(ctx context.Context) {
	if err := guo.Exec(ctx); err != nil {
		panic(err)
	}
}

func (guo *GenreUpdateOne) sqlSave(ctx context.Context) (_node *Genre, err error) {
	_spec := sqlgraph.NewUpdateSpec(genre.Table, genre.Columns, sqlgraph.NewFieldSpec(genre.FieldID, field.TypeInt))
	id, ok := guo.mutation.ID()
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "Genre.id" for update`)}
	}
	_spec.Node.ID.Value = id
	if fields := guo.fields; len(fields) > 0 {
		_spec.Node.Columns = make([]string, 0, len(fields))
		_spec.Node.Columns = append(_spec.Node.Columns, genre.FieldID)
		for _, f := range fields {
			if !genre.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("ent: invalid field %q for query", f)}
			}
			if f != genre.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, f)
			}
		}
	}
	if ps := guo.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if value, ok := guo.mutation.Name(); ok {
		_spec.SetField(genre.FieldName, field.TypeString, value)
	}
	if value, ok := guo.mutation.TypeID(); ok {
		_spec.SetField(genre.FieldTypeID, field.TypeUint32, value)
	}
	if value, ok := guo.mutation.AddedTypeID(); ok {
		_spec.AddField(genre.FieldTypeID, field.TypeUint32, value)
	}
	if guo.mutation.MoviesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   genre.MoviesTable,
			Columns: genre.MoviesPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(movie.FieldID, field.TypeInt),
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := guo.mutation.RemovedMoviesIDs(); len(nodes) > 0 && !guo.mutation.MoviesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   genre.MoviesTable,
			Columns: genre.MoviesPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(movie.FieldID, field.TypeInt),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := guo.mutation.MoviesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   genre.MoviesTable,
			Columns: genre.MoviesPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(movie.FieldID, field.TypeInt),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	_node = &Genre{config: guo.config}
	_spec.Assign = _node.assignValues
	_spec.ScanValues = _node.scanValues
	if err = sqlgraph.UpdateNode(ctx, guo.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{genre.Label}
		} else if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{msg: err.Error(), wrap: err}
		}
		return nil, err
	}
	guo.mutation.done = true
	return _node, nil
}
